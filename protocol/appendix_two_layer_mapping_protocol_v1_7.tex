...
% Version 1.8 applies amendment A004: the screening gate structure, the realigned
% eligibility criteria, the attrition stratum, and the quality-evidence extraction axes.
% Version 1.7 formalized manual-search provenance, raw documentary inventory, and auditability controls.
...
\chapter{Protocol for the Quality-Focused Systematic Mapping Study}
\label{app:quality_focused_mapping_protocol}

\section{Protocol Identification and Status}
\label{sec:protocol_identification}

This appendix specifies the protocol for a focused systematic mapping study with structured extraction and synthesis of quality evidence in LLM-based UML diagram generation. The study maps primary research on the generation, transformation, completion, repair, refinement, or revision of UML diagram content from textual software requirements and explicit domain descriptions, and analyzes extractable evidence about syntactic quality, semantic quality, syntax-semantic dissonance, evaluation references, measurement procedures, pragmatic adequacy, and correction effort.
...
\midrule
Review title & \reviewtitle: A Focused Systematic Mapping Study of Quality Evidence \\
Review type & Focused systematic mapping study with structured extraction and synthesis of quality evidence \\
Protocol version & 1.8, screening gate redesign draft (amendment A004) \\
...
Related secondary studies provide important but different forms of coverage. Reviews of UML model quality predate the current LLM based generation setting \cite{Genero2011UMLQuality}. Reviews of transformations from natural language to UML mainly cover rule based and earlier machine learning approaches \cite{Ahmed2022NaturalUML}. Recent studies map LLM applications across Model Driven Engineering or review UML and entity relationship diagram generation at a broad level \cite{Zhang2027LLMMDE,Rahmanian2026Diagrams}. The present study has a narrower and more analytically intensive objective. It maps LLM-based UML generation studies that report extractable quality evidence and synthesizes how they define, classify, measure, and validate diagram quality and inadequacies.
...
\section{Study Design and Analytical Focus}
\label{sec:protocol_design}

Systematic maps are primarily designed to structure a research area through classification and counting, whereas systematic reviews place greater emphasis on focused evidence synthesis \cite{Petersen2015Mapping}. This study combines descriptive mapping with structured extraction and synthesis of quality evidence in a single eligible corpus. Every included report must provide extractable evidence about the quality of generated UML content. Analytical groupings are derived from extraction fields, including the availability of syntactic evidence, semantic evidence, syntax-semantic dissonance evidence, and PlantUML generation; they are not separate eligibility strata.

\section{Objective and Expected Contribution}
\label{sec:protocol_objective}

The general objective is to map primary studies that use LLMs to generate, transform, complete, repair, refine, or revise UML diagram content from textual software requirements or explicit textual domain descriptions, and to synthesize how these studies characterize, operationalize, measure, and report the syntactic and semantic quality of the generated UML content, including evidence of syntax-semantic dissonance.
...
\begin{quote}
\textbf{ORQ.} How have primary studies used LLMs to generate, transform, complete, repair, refine, or revise UML diagram content from textual software requirements or explicit textual domain descriptions, and how do they characterize, operationalize, measure, and report syntactic quality, semantic quality, and syntax-semantic dissonance in the generated UML content?
\end{quote}
...
\begin{description}[style=nextline,leftmargin=2.0cm,labelwidth=1.5cm]
    \item[SQ1.] Which quality constructs are used, and how are textual validity, rendering validity, UML syntactic conformity, semantic correctness, semantic completeness, consistency, pragmatic adequacy, and correction effort defined or operationalized?
    ...
    \item[SQ3.] Which metrics, rubrics, oracles, semantic references, units of assessment, scales, thresholds, aggregation rules, evaluators, automated tools, and judgment procedures are used?
    ...
    \item[SQ6.] How are requirements, technical specifications, domain knowledge, UML instructions, prompts, examples, tools, retrieval, and feedback provided as generation context, and how are pragmatic adequacy, inspection, correction, rework, or productivity effects defined and measured?
\end{description}
...
SQ6 & Context variables, requirements, domain knowledge, UML instructions, prompts, examples, tools, feedback, task, effort measure & Narrative synthesis stratified by inferential strength & Generation context, pragmatic, and rework evidence map \\
\bottomrule
\end{longtable}
...
The search strategy is intentionally broad enough to identify candidate studies for the focused review. It is organized into three conceptual blocks.
...
The search does not require quality terms. Requiring terms such as semantic, correctness, or hallucination at retrieval time could exclude generation studies that evaluate quality under different terminology. Extractable quality evidence is therefore never used as an eligibility filter at any stage, and is instead classified after retrieval through structured full-text inspection on the axes of Section~\ref{sec:protocol_quality_axes}. Excluding studies that lack the review's evaluation vocabulary, whether at retrieval or at screening, would select on the review's own dependent variable and bias the map towards the studies that already speak the language the review exists to describe.
...
Backward and forward snowballing will be conducted according to a predefined and recorded procedure \cite{Wohlin2014Snowballing}. The start set comprises all included studies and relevant secondary studies. Backward snowballing inspects references. Forward snowballing uses Google Scholar and, when available, Scopus or Web of Science citation links. Newly identified records are deduplicated and screened with the same criteria used for database results. Iterations continue until one complete iteration produces no new eligible primary study.
...
\subsection{Screening gate structure}
\label{sec:protocol_gates}

Amendment A004 restructures screening so that every decision point carries exactly one question, at most one inclusion criterion, and at most one exclusion criterion. The criteria tables that follow are read against this structure: the code records the identity of the criterion, and the gate outcome field records where it was applied.

\begin{longtable}{p{0.05\textwidth} p{0.05\textwidth} p{0.46\textwidth} p{0.13\textwidth} p{0.13\textwidth}}
\caption{Screening gate structure after amendment A004}
...
Two structural asymmetries are deliberate. E3 has no positive counterpart, because it predicates a property of the record set rather than of the report. Filter B0 has no criterion of its own, being the conjunction of I4, I5, and I7; it absolves but does not condemn.

I8 is evaluated at two moments without receiving a separate code for each: at B5 from title and abstract, against mixture with another notation, and at C1 from the full text, against mixture with other artifacts, tasks, or outputs. The moment of evaluation is recorded in the gate outcome field. Creating one code per reading moment would enlarge the table without adding discriminating power.

\subsection{Exclusion criteria}

Exclusion codes are frozen. Codes withdrawn by amendment A004 leave the operative table and remain recorded as declared gaps, so that the numbering of decisions taken before the amendment is not rewritten and the trace of the amendment is not erased.
...
\begin{longtable}{p{0.08\textwidth} p{0.80\textwidth}}
\caption{Exclusion criteria withdrawn by amendment A004, kept as declared gaps}
\label{tab:protocol_exclusion_withdrawn}\\
\toprule
\textbf{Code} & \textbf{Disposition} \\
\midrule
\endfirsthead
\toprule
\textbf{Code} & \textbf{Disposition} \\
\midrule
\endhead
E5 & Withdrawn. Full-text unavailability is attrition, not ineligibility, and is handled by the stratum of Section~\ref{sec:protocol_attrition}. \\
E10 & Withdrawn. Split into filter C1 with code E12 and the extraction attribute \textit{result attribution}. \\
E11 & Withdrawn. Replaced by the three extraction axes of Section~\ref{sec:protocol_quality_axes}. Extractable quality evidence is no longer an eligibility requirement. \\
\bottomrule
\end{longtable}

Bare E7 always denotes the first clause of the criterion as originally published, namely that the generated result is not UML. The sibling clause carries the suffixed code E7b, whose suffix preserves its provenance. E12 receives a new number rather than the vacated E10 because its statement changed from a negative aggregate to a positive verifiable question, and a new statement receives a new code.

When more than one exclusion criterion applies, the first criterion that clearly explains the exclusion in filter order is recorded as primary, and the remaining ones are recorded in the justification. If title and abstract are insufficient to decide whether the report involves LLM-based UML generation from textual requirements or domain descriptions, the report is retained for full-text screening. The absence of explicit syntactic or semantic terminology is never sufficient for exclusion, at any gate, because the review's search was deliberately designed without quality terms and selecting on evaluation vocabulary would amount to selecting on the review's own dependent variable.
...
When title and abstract do not make the role of the model clear, the report is retained for full-text screening. The exclusion decision is made only when the full text provides sufficient evidence that the language model does not have a substantive role in producing, transforming, completing, repairing, refining, or revising UML diagram content.
...
The classification is recorded on three independent axes derived from the operational definitions of Section~\ref{sec:protocol_scope} and from the model quality relations of Lindland et al. and Krogstie \cite{Lindland1994Quality,Krogstie2006Process}.
...
The three axes are never collapsed into a single scale. Folding pragmatic adequacy into the domain axis would repeat the failure that produced an overloaded exclusion code during the first screening pass: a fused question yields a fused code, and the distinction becomes unrecoverable.

A fourth field, inherited from the withdrawn E10, records \textit{result attribution} with values \textit{attributable to UML}, \textit{aggregated with other artifacts}, and \textit{not reported}. It qualifies the strength with which a reported result can be traced to the generation of UML content, in cases where filter C1 confirmed that a generation instance exists.

The extraction additionally classifies whether the study provides evidence about textual validity, rendering validity, UML syntactic conformity, semantic correctness, semantic completeness, consistency, pragmatic adequacy, or correction effort. It records whether syntactic and semantic evidence are reported separately and whether the report provides direct, indirect, absent, unclear, or non-assessable evidence of syntax-semantic dissonance. Procedure, reliability, and validity information is recorded in the SQ5 extraction fields and never in the axes.
...
SQ1 to SQ3 & Studies for which axis D is not absent. \\
SQ4 & Studies for which axis L is not absent and axis D is not absent. \\
SQ6 & Studies for which axis U is not absent. \\
\bottomrule
\end{longtable}

SQ4 asks to what extent the reported evidence distinguishes syntactic from semantic quality. Under an eligibility filter on evidence reporting, that subset would be assumed; under the axes it is computed, which is the difference between describing the literature and describing the review's own filter. The same argument governs MQ5, which asks which combinations remain under-investigated: a gap analysis run over a corpus filtered by evidence reporting would report the filter as a gap in the literature.
...
Reports in the stratum are characterized rather than merely counted, using the metadata and abstracts available for them. The cut-off date for access attempts is unchanged, but its effect changes from excluding reports to closing the stratum: on that date the stratum stops receiving new texts and is counted and described. Question-level bases are stated explicitly, distinguishing the descriptive questions answerable over all retained reports from the questions that require full text, with a sensitivity check of the former against the abstracts of the non-retrieved stratum.
...
\midrule
\endhead
Axis & QualityAxisL & Absent; textual validity; UML conformity. Records the evidence reported about the relation between the model and its language. \\
Axis & QualityAxisD & Absent; claimed only; source requirements; reference model; expert judgment; rubric. Records the evidence reported about the relation between the model and the relevant domain. \\
Axis & QualityAxisU & Absent; claimed only; comprehension; engineering activity; rework. Records the evidence reported about the relation between the model and its interpreters or uses. \\
Axis & ResultAttribution & Attributable to UML; aggregated with other artifacts; not reported. Qualifies how far a reported result can be traced to the generation of UML content. Inherited from the withdrawn E10. \\
...
Construct & SemanticQualityConstruct & Semantic correctness; semantic completeness; requirements coverage; domain fidelity; consistency with source requirements; omitted element; unsupported addition; incorrect relation; incorrect actor; incorrect behavior; other; not reported. \\
...
Inadequacy & Normalized discrepancy operation & Omission, unsupported addition, incorrect representation, incorrect classification, inconsistency, duplication, redundancy, or emergent category. Selecting \textit{emergent category} requires recording the emergent label in free text \\
...
Evaluation & HumanEvaluatorRole & Author; independent evaluator; domain expert; UML expert; student; practitioner; crowd worker; not reported; not applicable. \\
...
\subsection{Generation context, pragmatic adequacy, and rework synthesis}

Reported context factors are classified as merely described, experimentally manipulated, statistically associated, or proposed without empirical evaluation. The synthesis examines how requirements, technical specifications, domain knowledge, UML technical instructions, prompts, examples, retrieval, tools, parser or renderer feedback, and iterative workflows are provided as generation context. Causal language is used only when the original design supports it. Pragmatic and rework evidence is synthesized by task, actor, diagram type, outcome, and measure. A study that reports comprehensibility without an engineering task is not automatically coded as evidence of pragmatic adequacy.
...
\midrule
Primary reviewer & Helaine Barreiros & Search execution, record management, complete screening, extraction coordination, synthesis, and audit log \\
Second reviewer & Emanoel Barreiros & Independent calibration, sampled title and abstract screening, complete full text screening, quality-evidence extraction and coding \\
Adjudicator & Ivaldir Hon\'orio & Resolution of unresolved eligibility, extraction, appraisal, and coding disagreements \\
...
\midrule
\endhead
1.0, \protocolfield{Date} & Initial systematic mapping protocol draft & Establish the complete methodological architecture before validation & Baseline draft; not registered \\
1.1, 2026-08-10 & Information source strategy, manual venue sets, and source retention rules & Separate automated corpus identification from manual coverage validation and make source decisions auditable & Improved coverage rationale and reproducibility; no change to review questions or analytical design \\
...
1.7, 2026-08-11 & Manual-search provenance and documentary-inventory controls & Formalize source roles, PRIMARY\_TOC membership, source manifests, permissible snapshots and hashes, immutable raw documentary inventories, raw-to-normalized identifier traceability, InventoryConflict reconciliation, operational unit states, and strengthened COMPLETE criteria & Makes the manual-search population and every transformation from documentary source to screening decision independently auditable; no change to research questions, eligibility criteria, analytical design, or review interval \\
1.8, 2026-08-17 & Screening gate redesign and eligibility criteria realignment (amendment A004) & Introduce the gate structure D, A1--A3, B0--B5, C1, with one question and at most one criterion per filter; renumber inclusion criteria I1--I8 in filter order while freezing exclusion codes; add E7b and E12; withdraw E5, E10, and E11 as declared gaps; convert full-text unavailability into the \textit{identified, not retrieved} attrition stratum; replace the evidence filter with the extraction axes L, D, and U plus the result-attribution attribute; compute the analytical subsets from the axes & Removes the only eligibility filter that operated on the review's dependent variable and the only criterion requiring exhaustive reading before it could be applied; no change to research questions, search strings, information sources, temporal and language scope, or the exclusion decisions already recorded \\
...
\midrule
\endhead
V01 & The focused systematic mapping design and quality-evidence emphasis are explicitly justified. & \protocolfield{Pending} \\
...
V04 & The unified eligibility criteria have positive and negative examples, one inclusion and one exclusion criterion per filter, and no filter operating on the review's dependent variable. & \protocolfield{Pending} \\
...
V10 & Mapping, generation-context, and quality-evidence extraction fields have been piloted and revised. & \protocolfield{Pending} \\
