...
Hallucination & Content in the generated diagram that the source does not support. Recorded as \textit{intrinsic} when it contradicts the input and \textit{extrinsic} when it cannot be verified from it. It is a deviation on the domain axis and never names an omission, which is incompleteness, and it is a term of the generation process reported as such by the study rather than a property the review infers from the artifact. \\
...
The word \textit{hallucination} is adopted where it belongs and refused where it does not. On axis D it is the established name, in both the software engineering and the natural language generation literatures, for content the source does not support, and it is recorded with the intrinsic and extrinsic distinction those literatures draw, intrinsic content contradicting the input and extrinsic content being unverifiable from it. It does not name omission. Evaluation practice keeps \textit{faithfulness} and \textit{completeness} as complementary criteria, so a diagram that leaves out what the source requires is incomplete and is not hallucinating, which is why omission stays paired with \textit{feasible completeness} and unsupported addition with \textit{feasible validity}. A further reason keeps hallucination off the umbrella: it predicates the generation process, whereas the axes predicate the artifact against a reference, and most reports observe the artifact rather than the model. Asserting the process property from artifact evidence is the attribution that field \textit{ResultAttribution} exists to qualify.
...
Deviation & Normalized discrepancy operation & Omission, unsupported addition, intrinsic hallucination, extrinsic hallucination, incorrect representation, incorrect classification, inconsistency, duplication, redundancy, or emergent category. The two hallucination values are available for the domain axis only and are never selected for an omission. These are \textit{deviations}, the dual of the normalized construct field above and never a repetition of it; Section~\ref{sec:protocol_attribute_deviation_measure} states the correspondence. Selecting \textit{emergent category} requires recording the emergent label in free text \\
