...
Understandability & Degree to which an interpreter correctly grasps what the diagram states about the domain. Its object is the interpreter, and it is assessed against what the diagram was meant to convey, not against how it looks. \\
Readability & Degree to which the concrete representation can be traversed: layout, crossings, labelling, and density. Its object is the representation, so a diagram may be readable and still be misunderstood. \\
Clarity & Degree to which what the diagram states admits a single reading. Its object is the statement, so a diagram may be readable, understood by one interpreter, and still unclear because a second interpreter reads it otherwise. \\
Deviation & A reported or inferable departure of the generated diagram from an explicitly identified language, semantic, or task reference. This is the umbrella term, and it is a count noun by design: a deviation is located in the diagram, is enumerable, and carries a violated reference, a carrier, a severity, and an evidence excerpt. Section~\ref{sec:protocol_attribute_deviation_measure} gives the axis-specific names and states why \textit{defect}, \textit{fault}, and \textit{anomaly} are not adopted. The review preserves the source term and separately assigns a normalized code. \\
Inadequacy & A deviation on the use axis: a failure of the diagram to support the defined engineering activity for a specified actor and context. It is the negative pole of \textit{pragmatic adequacy} above. It is not the umbrella for the other two axes, and not the \textit{adequacy} of natural language generation evaluation, which names the semantic axis \cite{Ji2023Hallucination}. \\
Hallucination & A failure of the generating model, reported by a study under that name. It is recorded as native vocabulary, exactly as the study writes it, and is not a normalized value of this review: the two literatures that use it disagree over whether an omission counts, and Section~\ref{sec:protocol_attribute_deviation_measure} states why the review records that disagreement instead of settling it. The review's own classification of a deviation comes from the reference it violates. \\
Syntax-semantic dissonance & A short label, declared once in Section~\ref{sec:protocol_attribute_deviation_measure}, for the measured co-occurrence of conformity on the language axis with deviation on the domain axis. It is not a quality construct and adds nothing the two axes do not already determine. \\
...
The three synthesis questions that carry the constitutive function of this review operate at three different levels, and those levels are the ones of the measurement vocabulary of systems and software engineering rather than a distinction introduced here. An \textit{attribute} is a property of an entity that can be distinguished quantitatively or qualitatively. Keeping the property apart from the procedure that assigns it a value is a long-standing requirement of conceptual model quality evaluation, not a distinction this review introduces \cite{Moody2005ConceptualQuality}. This review calls the attributes it extracts \textit{quality constructs}, which is the term of empirical Software Engineering for a property as a study names and operationalizes it; \textit{attribute} is the term of the measurement standards for the same level. The two are used as one, and \textit{dimension} is not a third name for it: a dimension here is one of the three axes, and nothing else. A \textit{measure} assigns a value to an attribute through a stated measurement method, and an \textit{indicator} is a measure that supports interpretation against decision criteria \cite{ISO15939,ISO25020,SWEBOK2024}. A \textit{deviation} is a departure of the entity from an explicitly identified reference. The normalized level is named \textit{measure} and not \textit{metric}, following the standard cited here, which replaced the older term when the SQuaRE series superseded ISO/IEC 9126 \cite{ISO25000,ISO25020}. \textit{Metric} is nevertheless kept in every field that records what a primary study reported, because that is the word the studies use and this review registers native vocabulary before normalizing it. Where the two appear together, \textit{metric} is the study speaking and \textit{measure} is the review. The review records, for every included study, a single spine.
...
SQ1 covers the first link, SQ2 the second, and SQ3 the remainder, restated as Equation~\ref{eq:protocol_metrology_chain} where the measure catalogue is specified. Keeping the links apart is the purpose of the decomposition. A primary study that reports having measured semantic correctness and gives a percentage has named an attribute and reported an aggregated value, and has usually left the deviation, the unit, and the denominator unstated. Recovering those is what this review adds, and it cannot be done by an instrument that stores the three levels in one field. Conflating a measurable concept with its measure is the specific failure the separation is designed to prevent.

The two normalization vocabularies are therefore duals, and they must not share labels. Every value of the normalized construct field names an attribute, and every value of the normalized discrepancy operation field names a departure from one. The correspondences are semantic completeness with omission; semantic correctness with unsupported addition, incorrect representation, and incorrect classification; consistency with inconsistency; and textual validity and UML syntactic conformity with the corresponding nonconformity in the sense of \cite{ISO9000}, which the violated reference field records at the level at which it occurred. A deviation that decomposes into an operation and a carrier, such as an incorrect relation or an incorrect actor, is recorded as that pair and never as a construct label, because storing it in both vocabularies would report a deviation type as a quality construct in the construct matrix of SQ1. Effort quantities such as correction effort and inspection effort are measures and are recorded in the rework fields, not as construct labels.

The duality is not an identity, and its asymmetries are findings rather than flaws of the instrument. A study may report deviations without ever naming the attribute they depart from, in which case the attribute is recorded as not reported, and a study may name an attribute and report no deviation at all. SQ1 and SQ2 are reported over the same corpus so that both asymmetries remain visible.

The axis-specific name of a deviation follows the reference it violates, because quality here is a relation and not a property of the diagram alone \cite{Lindland1994Quality,Krogstie2006Process}. On axis L a deviation is a nonconformity to the grammar of the concrete representation, or to the abstract syntax and well-formedness constraints of the modeling language \cite{OMG2017UML}. On axis D it is an invalidity when the diagram asserts what the reference does not support, and an incompleteness when the reference contains what the diagram omits. These are not analogies: they are the negative poles of the two semantic goals that the framework this protocol adopts calls \textit{feasible validity} and \textit{feasible completeness} \cite{Lindland1994Quality}, so the two deviation categories this review treats as central, omitting required domain elements and introducing unsupported content, are that pair and not a new invention. On axis U it is an inadequacy in the strict sense, namely a failure to support the defined engineering activity for the specified actor and context, which is the negative pole of the third goal of the same framework, \textit{feasible comprehension}. In process terms, axes L and D correspond respectively to verification against a specified reference and to validation against intended use \cite{ISO12207}. The phenomenon this review is built around is accordingly not a further quality construct. It is the co-occurrence of conformity on axis L with deviation on axis D, computed from the axes rather than asserted, and it is described in the vocabulary of the two literatures it joins: a \textit{hallucination that passes syntactic validation}, or in language-neutral terms syntactic conformity without semantic validity. \textbf{This review labels that co-occurrence \textit{syntax-semantic dissonance}, and the label is declared here once.} It names a measured co-occurrence and nothing else: it is not a construct, it carries no definition of its own beyond the two axes, and no extraction field depends on it that the axes do not already determine.

The word \textit{hallucination} is the established name for a failure of the generating model, and the two literatures this review draws on disagree about where that failure ends. In natural language generation evaluation, \textit{faithfulness} and \textit{completeness} are complementary criteria and an omission is incompleteness rather than hallucination \cite{Ji2023Hallucination,Maynez2020Faithfulness}. In software engineering taxonomies of code-generation hallucination, missing functionality is classified as hallucination under task requirement conflict, and that category is reported as the most prevalent one \cite{Liu2024CodeHallucination,Zhang2025CodeHallucination,Lee2025HallucinationTaxonomy}. \textbf{This review does not resolve the disagreement, and it must not: a terminological fault line between two literatures that both describe this phenomenon is one of the findings the review exists to record, not an assumption it may settle in advance.}

The instrument therefore separates the two layers it already separates everywhere else. The label a study uses is recorded as the study wrote it, so a report that calls an omission a hallucination is registered saying exactly that. The normalized operation is recorded beside it, and it keeps \textit{omission} apart from \textit{unsupported addition} for a reason that is independent of where hallucination is held to end: the two carry different denominators, an addition being counted against what was generated and an omission against what the reference contains, so merging them would destroy the denominator this review exists to reconstruct. Disagreement between the native label and the normalized operation is therefore data and not error, and it is reported as such.

Hallucination is not made a normalized value. Classifying by it would require this review to adopt one of the two boundaries, and the classification it needs is already available on an axis its own field provides: what the deviation violates, recorded in the violated reference field, which is the cut software engineering taxonomies themselves use \cite{Zhang2025CodeHallucination,Lee2025HallucinationTaxonomy}. A second reason keeps the word off the umbrella. It predicates the generation process, whereas the axes predicate the artifact against a reference, and most reports observe the artifact rather than the model. The review does not deny the causal link between the two, which other studies investigate; it declines to assert it from artifact evidence, which is the attribution that field \textit{ResultAttribution} exists to qualify.

The umbrella term is stipulated rather than borrowed, and the alternatives are declined for stated reasons. \textit{Defect} and \textit{fault} presuppose a specification that the generated diagram is not required to have, commit to repair, and carry the fault to failure chain of systems in operation \cite{Avizienis2004Dependability}. \textit{Anomaly} is tied to a reporting and disposition process that this review does not run. \textit{Nonconformity} is exact on axis L and wrong on the other two \cite{ISO24765,IEEE1044}. The product quality models of the SQuaRE series characterize a software product in operation, and do not supply the constructs at issue here, which concern a specification artifact evaluated against a domain reference; they are cited for the measurement framework and not as the quality model of this review \cite{ISO25010,ISO25020}.


\section{Objective and expected contribution}
\label{sec:protocol_objective}

The study objective is to map primary studies that use LLMs to generate, transform, complete, repair, refine, or revise UML diagram content from textual software requirements or explicit textual domain descriptions, to synthesize how these studies characterize, classify, operationalize, measure, validate, and report the syntactic, semantic, and pragmatic quality of the generated UML content, the deviations they report, and the evidence of hallucination that passes syntactic validation, and to establish the quality constructs, deviation categories, units of assessment, evaluation references, reliability practices, baselines, and correction effort measures required to formulate the empirical questions and instruments of the subsequent thesis stages.
...
\begin{enumerate}[label=\roman*)]
    ...
    \item a normalized taxonomy of deviations reported in the literature, while preserving the original terminology used by each primary study;
...
\begin{quote}
\textbf{ORQ.} How have primary studies used LLMs to generate, transform, complete, repair, refine, or revise UML diagram content from textual software requirements or explicit textual domain descriptions, under which generation context, and how do they characterize, classify, operationalize, measure, validate, and report the syntactic, semantic, and pragmatic quality of the generated UML content, the reported deviations, and the evidence of hallucination that passes syntactic validation?
\end{quote}
...
\begin{description}[style=nextline,leftmargin=2.0cm,labelwidth=1.5cm]
    ...
    \item[SQ2.] Which deviations are reported, how are they defined, against which violated reference is each one a departure, which UML elements or relations carry them, and how can the original categories be normalized without erasing diagram specific distinctions?
    ...
    \item[SQ4.] To what extent does the reported evidence distinguish syntactic quality from semantic quality, and what direct or indirect evidence is available of hallucination that passes syntactic validation, the co-occurrence this review labels syntax-semantic dissonance?
...
SQ2 & Original deviation labels, textual definitions, examples, affected UML carrier, reference violated & Hybrid deductive and inductive coding, constant comparison, and negative case analysis & Taxonomy of reported deviations \\
SQ3 & Reference, oracle, unit, formula, denominator, scale, threshold, aggregation, procedure & Metrological comparison and traceability analysis & Evaluation reference and measure catalogue \\
...
Evaluation & BaselineCondition & No baseline; human-authored model; rule-based or deterministic tool; another LLM; another version or configuration of the same LLM; ablation of the generation context; published benchmark or gold standard; other. Repeatable. Which quality constructs the study reports is not recorded here: it is given by the three axes and by the dimension partition of the normalized construct field. \\
...
Construct & Normalized construct & Single normalization vocabulary of quality \textit{attributes}, partitioned by dimension. Syntactic: textual validity; rendering validity; PlantUML parseability; UML syntactic conformity; metamodel conformance; well-formedness; notation-level validity. Semantic: semantic correctness; semantic completeness; requirements coverage; domain fidelity; consistency; consistency with source requirements. Pragmatic: pragmatic adequacy; understandability; readability; clarity, each defined in Table~\ref{tab:protocol_definitions} by the object it predicates, namely the activity, the interpreter, the representation, and the statement. Or other. Deviation labels are not admitted here and are recorded as a discrepancy operation and a carrier in the deviation fields below; effort quantities are not admitted here and are recorded in the rework fields. Section~\ref{sec:protocol_attribute_deviation_measure} states the duality. The two construct fields below are the syntactic and semantic projections of this vocabulary and admit no value absent from it. \\
...
Measure & SyntacticMetricOrProcedure & Exact metric, rubric, parser, compiler, rendering check, metamodel check, checklist, human judgment, automated tool, or narrative procedure reported by the study. \\
Measure & SemanticMetricOrProcedure & Exact metric, rubric, oracle, reference model, gold standard, requirements traceability procedure, expert judgment, checklist, human coding, automated comparison, or narrative procedure reported by the study. \\
...
Deviation & Original label and definition & Preserve the source label and its operational meaning \\
Deviation & Normalized discrepancy operation & Omission, unsupported addition, incorrect representation, incorrect classification, inconsistency, duplication, redundancy, or emergent category. \textit{Hallucination} is deliberately absent: it is recorded as the study's own label in the field above, and the review classifies by the violated reference instead. These are \textit{deviations}, the dual of the normalized construct field above and never a repetition of it; Section~\ref{sec:protocol_attribute_deviation_measure} states the correspondence. Selecting \textit{emergent category} requires recording the emergent label in free text \\
Deviation & Violated reference & Concrete representation, UML abstract syntax, UML construct semantics, domain or requirements semantics, cross diagram consistency in the sense of the UML consistency-management literature \cite{Lucas2009UMLConsistency}, or intended engineering task \\
Deviation & UML carrier & Affected diagram, element, relation, property, constraint, sequence, state, transition, message, behavior, or other type specific unit \\
Deviation & Reported severity or task effect & Severity scale or task effect as the study reports it, in the study's own terms. This field exists because Equation~\ref{eq:protocol_deviation_tuple} makes severity the fourth dimension of the coding tuple; it is left empty when the study reports none, and emptiness is not imputed \\
Measure & Metric name and formula & Extract formula, numerator, denominator, scale, direction, threshold, and aggregation rule \\
Measure & Automation level & Fully automated, tool assisted, human coded, LLM assessed, or mixed \\
...
Every extractable deviation is represented, where evidence permits, as a tuple

\begin{equation}
D = \langle R_v, O_d, C_u, S_e, E_x \rangle,
\label{eq:protocol_deviation_tuple}
\end{equation}
...
\begin{itemize}
    ...
    \item quality construct assessed, as given by the three evidence axes and by the dimension partition of the normalized construct, by diagram type;
...
\subsection{Taxonomy of reported UML deviations}

The taxonomy must be presented at two levels. The transversal level organizes discrepancy operations and violated references that recur across diagram types. The specific level records how these operations manifest in actors, use cases, classes, relations, attributes, messages, lifelines, states, transitions, activities, and other UML elements. Frequencies are presented as frequencies of reporting and coding within the reviewed corpus, not as estimates of the real world prevalence of deviations unless the primary data support that inference. The taxonomy reports the number of distinct original labels and the number of normalized discrepancy operations they collapse into, on the same terms.

\subsection{Measure and evaluation-reference catalogue}

Measures and procedures are organized by the measurement tail of the spine of Equation~\ref{eq:protocol_quality_spine}, defined in Section~\ref{sec:protocol_attribute_deviation_measure}, namely the chain
...
The synthesis compares parsability, compilation, UML rule checks, precision, recall, F scores, coverage, edit distance, graph similarity, expert ratings, LLM-as-judge scores, task performance, time, and other measures only after reconstructing their definitions and denominators. Measures with the same name but different operationalizations are not pooled.
...
Pole confusion & A deviation type may be recorded as a quality attribute, so that the construct matrix reports deviation types as quality constructs of the literature & The two normalization vocabularies are disjoint duals, stated in Section~\ref{sec:protocol_attribute_deviation_measure} and enforced by a lock over the codebook \\
Stipulated terminology & \textit{Deviation} carries the umbrella role and \textit{inadequacy} is narrowed to the use axis, so a reader may still import the wider sense the word has elsewhere & The stipulation names both excluded readings, the declined standard alternatives are justified, and each deviation is additionally named by the axis of the reference it violates \\
...
Unit misalignment & Two reviewers may enumerate different numbers of deviations from one study, so that an agreement coefficient would measure how much each found rather than how far they agree & Unitization is reconciled before classification, the reconciliation is reported as a count, and only classification carries a coefficient \\
...
Reporting bias & Studies may omit unsuccessful generations, deviations, parameters, or evaluator disagreement & Appraisal of reporting, missing data coding, artifact inspection, author contact when essential, and cautious inference \\
...
The final report will avoid causal or prevalence claims that exceed the designs and units represented in the primary studies. Frequencies in the map describe the literature, not necessarily the real frequency of deviations in LLM generated UML diagrams. The taxonomy describes reported and operationalized deviations, not a complete ontology of every deviation that can occur.
