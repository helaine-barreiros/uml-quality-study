...
H = open(os.path.join(BASE, 'analysis/v3_0/ficha_extracao.html'), encoding='utf-8').read()
COD = list(csv.DictReader(open(os.path.join(BASE, 'analysis/v3_0/extraction/codebook_extracao.csv'),
                               encoding='utf-8')))
CABD = next(csv.reader(open(os.path.join(BASE, 'analysis/v3_0/extraction/extracao.csv'),
                            encoding='utf-8')))
falhas = []
...
print('== estrutura ==')
ok(len(camp) == 65, 'campos embutidos: %d (64 + NotationFamily)' % len(camp))
...
ok(sum(len(v) for v in donos.values()) == 65, 'todo campo tem exatamente um dono ou e instrumento')
