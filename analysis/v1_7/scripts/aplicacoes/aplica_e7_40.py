...
REV='HB'
IDS=[l.strip() for l in open('/tmp/g_semuml.txt') if l.strip()]
...
def nota(r,i):
    ...
    p.append('CONTEXTO: decidido em revisao do bloco de 61 propostas automaticas de E9, no qual 40 '
             'registros nao apresentavam nenhuma mencao a UML e foram reclassificados para E7 em B1. '
             'A decisao apoia-se em titulo, resumo e palavras-chave; caso o texto completo revele '
...
with open(LOG,'a',newline='',encoding='utf-8') as fh:
    csv.writer(fh).writerow([';'.join(IDS),AGORA,REV,'REVISAO_DECISAO','B1','E9 (proposta do script)','E7',
     'Reclassificacao em lote de 40 registros que o script de apoio propusera excluir por E9 no B4. '
     'O script disparou por detectar sinal de entrada nao textual (codigo-fonte, repositorio, imagem, '
     'log) sem antes confirmar o portao da saida, invertendo a ordem do fluxograma. Verificada a '
     'varredura dos tres campos, nenhum dos 40 apresenta uma unica ocorrencia de UML ou de tipo de '
     'diagrama do metamodelo UML, de modo que nao ha componente UML separavel e o B1 decide antes de '
     'o B4 ser alcancado. Aplicada a regra de ouro 3 (criterio primario e o primeiro portao que '
     'explica claramente o caso) e a regra de ouro 5 (falha de saida e I5/E7, nao E6 nem E9). Em '
     'varios deles o I2 e o I4 estao satisfeitos. Todos os 40 possuem resumo, portanto nenhuma '
     'exclusao se apoia em dado ausente. Restam 21 registros do bloco original, que tem mencao a UML '
     'e serao lidos individualmente.',
